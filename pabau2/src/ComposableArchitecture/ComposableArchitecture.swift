...
//      .removeDuplicates()
	...
	public func scope2<LocalValue, LocalAction>(
			value toLocalValue: @escaping (Value) -> LocalValue?,
			action toGlobalAction: @escaping (LocalAction) -> Action
		) -> Store<LocalValue, LocalAction>? {
			guard let localV = toLocalValue(self.value) else { return nil }
			let localStore = Store<LocalValue, LocalAction>(
				initialValue: localV,
				reducer: { localValue, localAction, _ in
					self.send(toGlobalAction(localAction))
	...
	public protocol StoreView: View {
		...
		var store: Store<Value, Action> { get }
		init(store: Store<Value, Action>)
	}

	extension Store {
		public func makeStoreViewOrEmpty<LocalValue, LocalAction, FinalView: StoreView>(
			type: FinalView.Type,
			value toLocalValue: @escaping (Value) -> Optional<LocalValue>,
			action toGlobalAction: @escaping (LocalAction) -> Action
		) -> some View where LocalValue == FinalView.Value, LocalAction == FinalView.Action {
			let localValue = toLocalValue(self.value)
			let localStore = self.scope2(value: { _ in localValue },
																	 action: toGlobalAction)
			return ViewBuilder.buildBlock(
				(localStore != nil) ?
					ViewBuilder.buildEither(second: type.init(store: localStore!))
					:
					ViewBuilder.buildEither(first: EmptyView())
			)
		}
		
	//	public func makeStoreView<LocalValue, LocalAction, FinalView: StoreView>(
	//		value toLocalValue: @escaping (Value) -> LocalValue,
	//		action toGlobalAction: @escaping (LocalAction) -> Action
	//	) -> some StoreView where LocalValue == FinalView.Value, LocalAction == FinalView.Action {
	//		let localStore = self.scope(value: toLocalValue,
	//																action: toGlobalAction)
	//		return FinalView.init(store: localStore)
	//	}
	}
