	...
	var body: some View {
		List {
			ForEach(self.state.indices, id: \.self) { idx in
				FinancialRow(financial: state[idx])
			}
		}
	}
}
...
private struct FinancialRow: View {
	...
	var body: some View {
		VStack {
			InvoiceRow(text1: Texts.invoice, invoice: financial.number, date: financial.date)
			...
			FinancialsTextRow(text1: Texts.amount, text2: financial.method)
		}
	}
}
