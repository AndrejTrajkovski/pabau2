...
let chooseServiceReducer =
	Reducer<ChooseServiceState, ChooseServiceAction, JourneyEnvironemnt> { state, action, _ in
	switch action {
...
struct ChooseService: View {
	...
	var body: some View {
		VStack {
			...
			List {
				ForEach(self.viewStore.listServices, id: \.self.first?.categoryId) { (group: [Service]) in
					Section(header:
